...
class Servo_SpeedDelay(threading.Thread):
    ...
    def run(self):
        ...
        while self.__stop == False:
            ...
            #self.logger.info("new value : %i, modified : %i" % (newvalue, modified))

            # Sent if new value
            if self.__servo.needSync():
                ...
                # Callback
                for (i, val) in enumerate(self.__values):
                    if i in modified and self.__values[i].current == self.__values[i].setpoint:
                        self.__callback(i)
